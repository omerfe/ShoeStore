...
namespace ApplicationCore.Services
{
    public class OrderService : IOrderService
    {
        ...
        public async Task<Order> CreateOrderAsync(int basketId, Address shippingAdress)
        {
            var spec = new BasketWithItemsSpecification(basketId);
            var basket = await _basketRepo.FirstOrDefaultAsync(spec);
            if (basket is null) throw new ArgumentException($"Basket with id {basketId} cannot be found.");
            if (basket.Items.Count == 0) throw new ArgumentException($"Basket does not contain any items.");
