...
namespace Web.Services
{
	public class BasketViewModelService : IBasketViewModelService
	{
		...
		public async Task<BasketViewModel> AddToBasketAsync(int productId, int quantity)
		{
			...
			// TODO: ApplicationCore'da BasketService : IBasketService oluştur ve AddItemToBasket metodunu yaz burada kullan ve geriye dönen Basket nesnesini BasketViewModel'a dönüştür ve geriye döndür.
			return null;
		}

		public async Task<Basket> GetOrCreateBasketAsync()
		{
			var buyerId = await GetOrCreateBuyerId();
		...
		public async Task<string> GetOrCreateBuyerId()
		{
			var userId = GetLoggedInUserId();
			if (userId is not null) return userId;

			var anonId = GetAnonymousBuyerId();
			if (anonId is not null) return anonId;
